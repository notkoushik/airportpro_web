...
// Static import to avoid dynamic import issues
...
const ProfessionalPassportScanner: React.FC<ProfessionalPassportScannerProps> = ({ onScanSuccess, onScanFailure } = {}) => {
  ...
  // FIXED: Enhanced MRZ extraction with proper parsing
  const extractMRZ = async (imageDataUrl: string): Promise<PassportData | null> => {
    try {
      console.log('🔍 Starting enhanced MRZ extraction...');
  ...
  // FIXED: Enhanced MRZ line detection
  const extractMRZLines = (text: string): string[] => {
    ...
    for (const line of lines) {
      ...
      if (cleanLine.length >= 36 && cleanLine.includes('<')) {
        // Validate it looks like MRZ (starts with P< for passport)
  ...
  // FIXED: Enhanced MRZ parsing with correct field extraction
  const parseEnhancedMRZ = (mrzLines: string[]): PassportData | null => {
    try {
      ...
      // FIXED: Parse first line - P<COUNTRY<SURNAME<<GIVENNAMES
      ...
      // FIXED: Parse second line with correct field positions
      // Format: PASSPORTNUMBER<CHECK<BIRTHDATE<CHECK<GENDER<EXPIRYDATE<CHECK<PERSONALNUMBER<<<CHECK
      let documentNumber = '';
      let birthDate = '';
      let gender = '';
      let expiryDate = '';
      ...
      if (line2.length >= 44) {
        ...
        birthDate = line2.substring(13, 19); // YYMMDD format
        // Skip check digit at position 19
        gender = line2.charAt(20) || 'U';
        expiryDate = line2.substring(21, 27); // YYMMDD format
        // Skip check digit at position 27
        personalNumber = line2.substring(28, 42).replace(/</g, '').trim();
      }
      ...
      // FIXED: Enhanced nationality mapping
  ...
  // FIXED: Enhanced country name mapping
  const getEnhancedCountryName = (code: string): string => {
    const countries: { [key: string]: string } = {
      ...
      'SAU': 'Saudi Arabia',
      'NPT': 'Nepal', // FIXED: Added Nepal mapping
      'BGD': 'Bangladesh',
      'PAK': 'Pakistan',
      'LKA': 'Sri Lanka',
      'UNK': 'Unknown'
  ...
  const takePicture = async () => {
    try {
      ...
      console.log('📷 Starting professional passport scan...');
      ...
      if (image.dataUrl) {
        ...
        // Extract passport data with enhanced parsing
        ...
        if (passportData && passportData.surname !== 'UNKNOWN' && passportData.documentNumber !== 'UNKNOWN') {
          const result: ScanResult = {
            ...
            ocrMethod: 'Professional Tesseract.js OCR'
          };
          ...
          const result: ScanResult = {
            ...
            timestamp: Date.now(),
            imageData: image.dataUrl,
            ocrMethod: 'Professional OCR - Failed'
          };
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="text-center py-6">
          ...
          <div className="flex items-center justify-center space-x-2">
            <p className="text-gray-600">Enhanced OCR • Real Data Extraction</p>
        ...
        <Card className="shadow-xl border-0 bg-white backdrop-blur">
          <CardHeader className="text-center pb-6">
            ...
            <CardDescription className="text-base">
              AI-powered MRZ extraction with enhanced parsing accuracy
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            ...
            <div className="aspect-[4/3] bg-gradient-to-br from-gray-100 to-gray-200 rounded-xl flex items-center justify-center border-2 border-dashed border-gray-300 overflow-hidden">
              {imagePreview ? (
                ...
                <div className="text-center">
                  ...
                  <p className="text-lg font-medium text-gray-700">Processing with Professional OCR</p>
                  <p className="text-sm text-gray-500 mt-2">Enhanced MRZ parsing in progress</p>
                </div>
              ) : (
            ...
            {/* FIXED: Scan Button with proper styling */}
            <Button 
              onClick={takePicture} 
              disabled={scanning}
              size="lg"
            ...
            >
              {scanning ? (
                <>
                  ...
                  Analyzing with Professional OCR...
                </>
              ) : (
                <>
        ...
        {scanResult && (
          <div ref={resultRef} className="space-y-6">
              ...
              <CardContent className="pt-6">
                <div className="flex items-center justify-center space-x-3 mb-4">
                  ...
                  <h2 className={`text-2xl font-bold ${scanResult.success ? 'text-green-800' : 'text-red-800'}`}>
                    {scanResult.success ? 'Professional Scan Successful!' : 'Professional Scan Failed'}
                  </h2>
                </div>
            ...
            {/* FIXED: Action Buttons with proper contrast */}
            {scanResult.success && (
            ...
            {!scanResult.success && (
              <Card className="border-red-200 bg-red-50">
                <CardContent className="pt-6">
                  <h3 className="text-lg font-bold text-red-800 mb-4">Professional Scan Failed</h3>
                  <div className="space-y-2 text-red-700">
                    <p>• No clear MRZ (Machine Readable Zone) detected</p>
                  ...
                  {scanResult.data.rawMRZ && scanResult.data.rawMRZ !== 'No clear MRZ detected in image' && (
                    <div className="mt-4 p-3 bg-red-100 rounded-lg">
                      ...
                      <pre className="text-red-700 text-xs whitespace-pre-wrap">{scanResult.data.rawMRZ}</pre>
                    </div>
                  )}
